...
class InductiveKnowledgeGraphDataset(data.KnowledgeGraphDataset):
    ...
    def load_inductive_tsvs(self, train_files, test_files, verbose=0, rels=None):
        verbose = 0
        ...
        for txt_file in train_files:
            ...
            with open(txt_file, "r") as fin:
                ...
                for tokens in reader:
                    ...
                    #TESTESTEST
                    if rels == r_token and np.random.rand() < sample_rate:
                        self.focus_triples.append(num_sample)
...
class FB15k237Inductive(InductiveKnowledgeGraphDataset):
    ...
    train_urls = [
        ...
        "grail_sets/train/train.txt",
        "grail_sets/train/valid.txt"
    ]

    test_urls = [
        #"https://raw.githubusercontent.com/kkteru/grail/master/data/fb237_%s_ind/train.txt",
        #"https://raw.githubusercontent.com/kkteru/grail/master/data/fb237_%s_ind/test.txt",
        "grail_sets/test/train.txt",
        "grail_sets/test/test.txt"
    ]
    ...
    def __init__(self, path, version="v1", verbose=1):
        ...
        for url in self.train_urls:
            ...
            txt_file = os.path.join(path, url) 
            if not os.path.exists(txt_file):
                print("BAD TRAIN PATH: {}".format(url))
                txt_file = utils.download(url, self.path, save_file=save_file)
            print("TRAIN FILE: "+txt_file)
        ...
        for url in self.test_urls:
            ...
            txt_file = os.path.join(path, url) 
            if not os.path.exists(txt_file):
                print("BAD TEST PATH: {}".format(url))
                txt_file = utils.download(url, self.path, save_file=save_file)
            print("TEST FILE: "+txt_file)
            test_files.append(txt_file)

        print("CHECKING: {}".format(self.test_urls[1]+"_rels"))
        txt_file = os.path.join(path, self.test_urls[1]+"_rels") 
        ...
        print("*"*20+"\n\n")
        self.load_inductive_tsvs(train_files, test_files, verbose=verbose, rels=rel)
