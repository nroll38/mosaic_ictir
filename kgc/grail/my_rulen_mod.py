    ...
    for j in tqdm(range(100)):
        ...
        while(ret==None):
            ...
            if cur_sub > 0 and i == 0:
                ...
                #print(all_found_list)
                #try:
                #print("To del: {}".format(all_found_list[r][sub-cur_sub][1]))
                #print(all_found_list)
                #exit()
                data, data_s, rem_data_subs = remove_r(data, data_s, rem_data_subs, s, all_found_list[r][0][1], o)
                #data, data_s, rem_data_subs = remove_r(data, data_s, rem_data_subs, s, [top_rule_list[0][0]], o)
                #data, data_s, rem_data_subs = remove_r(data, data_s, rem_data_subs, s, all_found_list[r][sub-cur_sub][1], o)
                #except:
                #    print("WHY?")
                #    continue
...
def remove_pred(data_fp, mod_fp):
    o_data, o_data_s = load_data(data_fp)
    cn_rem, cn_rem_s = load_data(mod_fp)
    rem_data_targ = {}
    rem_data_subs = {}
    random.seed(123)
    count = 0
        ...
        for i_r in cn_rem:
            for i_s in cn_rem[i_r]:
                for i_o in cn_rem[i_r][i_s]:
                    ...
                    num, found_list = rulen_mod(i_s, i_r, i_o, rules[i_r], data, data_s, max_sub=0)
                    

                    if found_list:
                        count+= 1
                        found_list.sort(key=lambda x: x[2], reverse=True)
                        
                        to_remove[i_s] = found_list[0][1][0]
                        print(found_list[0][1][0])
                        #data, data_s, rem_data_subs = remove_r(data, data_s, rem_data_subs, s, all_found_list[r][0][1], o)
        

    with open(data_fp, "r") as fd:
        to_write = []
        rd = csv.reader(fd, delimiter="\t")
    ...
    print(len(to_write))
    count = 0
    with open(data_fp, "w") as fd:
        writ = csv.writer(fd, delimiter="\t")
        for row in to_write:
            if row[0] in to_remove and row[1] == to_remove[row[0]]:
                count += 1
                continue
            writ.writerow(row)
...
if __name__ == "__main__":
    ...
    test_fp = "./data/fb237_v1/test.txt"
    #do_split(data_fp, mod_fp)
    #do_single(data_fp, mod_fp)
    remove_pred(data_fp, test_fp)
