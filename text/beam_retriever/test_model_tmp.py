...
def get_retr_output(test_raw_data, type='musique', is_dev=True, beam_size=1, args=None):
    retr_dic = {}
    #if type == '2wiki':
    ...
    re_checkpoint=args.retriever_checkpoint

    pred_filename = "pred_test_{}_retr.json".format(args.name)#f"pred_test_{type}_v0_retr.json"
    max_len = 512
...
def get_reader_qa_output(retr_pred_dic, test_raw_data, type='musique', is_dev=True, answer_merge=False, topk=5, args=None):
    qa_tokenizer_path = "microsoft/deberta-v3-large"#"model/deberta-v3-large-squad2"
    ...
    qa_checkpoint = args.reader_checkpoint
    pred_filename = "_pred_test_{}.json".format(args.name)
    max_len = 1024
    device = torch.device("cuda", 0)#1)
    ...
    #Reader(bert_config, args.model_name, task_type=args.dataset_type, len_tokenizer=len(tokenizer)
    qa_model = Reader(config, qa_model_path, task_type=type, len_tokenizer=len(tokenizer))# if ('deberta' not in qa_tokenizer_path) else 0)
    ...
    for sample in tqdm(test_raw_data, desc="QA Predicting:"):
        ...
        if type == 'hotpot':
            ...
            print(outputs)
            if "output_answer_type" in outputs:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Beam Retriever test settings.')

    parser.add_argument('--type', type=str, default="hotpot", help='Dataset type')
    parser.add_argument('--reader_checkpoint', type=str, default="./reader_checkpoint.pt", help='Reader model checkpoint.')
    parser.add_argument('--retriever_checkpoint', type=str, default="./retriever_checkpoint.pt", help='Retriever model checkpoint.')
    parser.add_argument('--file_path', type=str, default="./test.json", help='Dataset test file path.')
    parser.add_argument('--name', type=str, default="base", help='Output name.')
    ...
    retr_json = get_retr_output(test_raw_data, is_dev=is_dev, type=type, beam_size=2, args=args)
    get_reader_qa_output(retr_json, test_raw_data, is_dev=is_dev, type=type, answer_merge=True, topk=10, args=args)
