...
CUDA_VISIBLE_DEVICES=0 python train_reader.py --do_predict --init_checkpoint ./reader_checkpoint.pt --prefix hotpot_reader_deberta_large --model_name microsoft/deberta-v3-large --tokenizer_path microsoft/deberta-v3-large --dataset_type hotpot --train_file ../hotpot_train_v1.1.json --predict_file ../data/new_dev.json --train_batch_size 8 --learning_rate 1e-5 --fp16 --max_seq_len 1024 --num_train_epochs 16 --predict_batch_size 1 --warmup-ratio 0.1 --log_period_ratio 0.01 --eval_period_ratio 0.3
mv reader_pred.json preds/base_pred.json

for i in {1..31}
do

    CUDA_VISIBLE_DEVICES=0 python train_reader.py --do_predict --init_checkpoint ./reader_checkpoint.pt --prefix hotpot_reader_deberta_large --model_name microsoft/deberta-v3-large --tokenizer_path microsoft/deberta-v3-large --dataset_type hotpot --train_file ../hotpot_train_v1.1.json --predict_file ../data/${i}dev.json --train_batch_size 8 --learning_rate 1e-5 --fp16 --max_seq_len 1024 --num_train_epochs 16 --predict_batch_size 32 --warmup-ratio 0.1 --log_period_ratio 0.01 --eval_period_ratio 0.3
    mv reader_pred.json preds/${i}_pred.json
